...
class Tuner extends Component {
    ...
    playNote = (evt) => {
        if(this.state.context && !this.state.drone) {
            this.stopNote();
        } 
    ...
    render() {
        return (
            ...
            <input checked={this.state.drone === true} onChange={event => this.toggleDrone()} className={classes.checkboxHide} type="checkbox" id="drone"/><label htmlFor="drone" className={classes.checkContainer}><span>Drone</span> </label>
            <input className={classes.checkboxHide} type="checkbox" id="instrument"/>    <label htmlFor="instrument" className={[classes.checkContainer, classes.toggleInstrument].join(' ')}><span>Guitar</span> </label>
            </div>
            </React.Fragment> 
        )
    }
